...
fn regenerate_certs() {
  ...
  let root_ca =
    issuer_params("Rustls Robust Root").self_signed(&root_key).unwrap();
  ...
  let mut end_entity_params =
    CertificateParams::new(vec![TEST_SERVER_DOMAIN.to_string()]).unwrap();
  ...
  chain_file.write_all(intermediate_ca.pem().as_bytes()).unwrap();

  let mut key_file = File::create("tests/certs/end.key").unwrap();
  key_file.write_all(end_entity_key.serialize_pem().as_bytes()).unwrap();
}
